...
const StakingDypiusBase1 = ({
  ...
  const refreshBalance = async () => {
    ...
    try {
      ...
      if (chainId === "8453" && coinbase && is_wallet_connected) {
        _bal = reward_token_dypiusv2_base.balanceOf(coinbase);
  ...
  const handleApprove = async (e) => {
    ...
    await reward_token_dypiusv2_base
  ...
  const checkApproval = async (amount) => {
    const result = await window
      .checkapproveStakePool(
        coinbase,
        reward_token_dypiusv2_base._address,
        staking._address
      )
      .then((data) => {
        console.log(data);
  ...
  const getAvailableQuota = async()=>{
    ...
    const stakingSc = new window.baseWeb3.eth.Contract(window.CONSTANT_STAKING_DYPIUS_ABI, staking._address)
        ...
        <div className="d-flex flex-column w-100 gap-2">
          ...
          <div
            className={`d-flex flex-column w-100 gap-1 ${
              (chainId !== "8453" || !is_wallet_connected) && "blurrypool"
            } `}
          >
              ...
              <div className="d-flex flex-column">
                ...
                <div className="d-flex align-items-center gap-2">
                  ...
                  <a
                    ...
                    href={`https://basescan.org/address/${staking?._address}`}
          ...
          {is_wallet_connected && chainId === "8453" && (
            <button
            disabled={
...
export default StakingDypiusBase1;
