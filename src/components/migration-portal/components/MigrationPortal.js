...
export function MigrationPortal({
  ...
  const [tokenAddress, setTokenAddress] = useState(
    window.config.token_dyp_migration_eth_address,
  );
  ...
  const getTokenBalance = async (token, sourceChain) => {
    if (!coinbase) return;
    const tokenAddress = window.config.token_dyp_migration_eth_address;
  ...
  const getAllDepositsByUser = async () => {
    ...
    const depositCounts_dyp = await contract_eth.methods
    ...
    let dypAmount = await contract_eth.methods
    ...
    let dypAmountFormatted = Web3.utils.fromWei(dypAmount.toString(), "ether");

    const depositCounts_idyp = await contract_bnb.methods
      .depositsCount(coinbase)
      .call()
      .catch((e) => {
        console.error(e);
    ...
    let idypAmount = await contract_bnb.methods
      .totalDepositedByToken(coinbase, window.config.token_idyp_eth_address)
      .call()
      .catch((e) => {
        return 0;
      });
    let idypAmountFormatted = Web3.utils.fromWei(
      idypAmount.toString(),
      "ether",
    );

    setIdypDeposits((prev) => ({
      ...prev,
      bnb: {
        amount: Number(idypAmountFormatted),
        count: Number(depositCounts_idyp),
      },
    }));
    setDypDeposits((prev) => ({
      ...prev,
      eth: {
        ...
        count: Number(depositCounts_dyp),
      },
    }));
  };
  ...
  useEffect(() => {
    if (selectedToken === "DYP") {
      setTokenAddress(window.config.token_dyp_migration_eth_address);
                      ...
                      title={
                        <div className="tooltip-text">
                          <div className="space-y-2">
                            ...
                            <p className="text-xs">
                              • After snapshot: standard rate applies (4-5x
                              lower)
                            </p>
                          </div>
                        </div>
                      ...
                      title={
                        <div className="tooltip-text">
                          <div className="space-y-2">
                            ...
                            <p className="text-xs">
                              Current rate is based on 30-day average (4-5x
                              lower than premium)
                            </p>
                          </div>
                        </div>
          ...
          <div className="bg-[#2a2b5e]/80 backdrop-blur-sm bordertw border-[#5a5b8c] rounded-xl p-6">
            ...
            {isConnected && coinbase && networkId === selectedChain.chainId && (
              <button
                ...
                className={`${
                  depositStatus === "initial" || depositStatus === "deposit"
                    ? "bg-gradient-to-r from-[#7c7df5] to-[#5a5bf5] hover:from-[#6b6ce5] hover:to-[#4a4be5]"
                    : depositStatus === "success"
                      ? "bg-gradient-to-r from-brandBlue to-brandCyan hover:from-blue-700 hover:to-cyan-600 pe-none"
                      : "pe-none d-flex align-items-center gap-2 justify-content-center bg-gradient-to-r from-amber-800 to-amber-1000 hover:from-orange-400 hover:to-orange-500"
                } w-full disabled:from-gray-600 disabled:to-gray-700 disabled:cursor-not-allowed text-white py-3.5 rounded-xl transition-all shadow-lg shadow-[#7c7df5]/20`}
              >
                {depositLoading ? (
                  <div
