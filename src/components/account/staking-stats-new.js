...
export default class StakingStats extends React.Component {
  constructor(props) {
    ...
    window.LP_ID_LIST_V2.forEach(
      (lp_id) => (this.state.pools_info[lp_id] = {})
    );
  }
  ...
  getDepositedDyp = async (contractAddress) => {
    ...
    for (let id of lp_id) {
      ...
      if (contractAddress == contractAdd) {
        constantStaking = new window.web3.eth.Contract(
  ...
  getClaimableTokens = async (contractAddress) => {
    ...
    else return await constantStaking.methods.getPendingDivs(coinbase).call();
  };
  ...
  handleClaim = async (contractAddress) => {
    let constantStaking = "";
    let lp_id = window.FarmingStakingAddresses;
    let farming = new window.web3.eth.Contract(
    ...
    let amount = await constantStaking.methods
      .getTotalPendingDivs(address)
      .call();
  ...
  refreshPoolsInfo = () => {
    ...
    window.LP_ID_LIST_V2.forEach(async (lp_id) => {
      ...
      let [
        ...
        this.getDepositedLP(contractAddress),
        this.getClaimableTokens(contractAddress),
        this.getClaimableWeth(contractAddress),
        this.getWethPaidOut(contractAddress),
        this.getTotalEarnedWeth(contractAddress),
        this.getDepositedDyp(contractAddress),
      ]);
  ...
  getTotalUsdValueOfLpDeposited = () => {
    ...
    for (let lp_id of window.LP_ID_LIST_V2) {
      let pool_info;
      if (!(pool_info = this.state.pools_info[lp_id])) continue;
      let usdValueLp =
        (pool_info.depositedLp / 1e18) * lp_data[lp_id].usd_per_lp;
      if (usdValueLp) {
        usdValue += usdValueLp;
  ...
  render() {
    ...
    let can_access =
      this.state.coinbase &&
      ACCESS_WALLET_LIST.includes(String(this.state.coinbase).toLowerCase());
          ...
          <div className="l-box pl-0 pr-0">
            ...
            >
              <table className="table">
                ...
                <tbody>
                  {window.LP_ID_LIST_V2.map((lp_id, i) => {
                    let usd_per_lp = the_graph_result.lp_data
                      ? the_graph_result.lp_data[lp_id].usd_per_lp
                      : 0;
                    ...
                    if (res[0] == "avax") addressImg = "/images/avax.png";
                    if (res[0] == "weth") addressImg = "/images/ETH.png";
                    // if (res[0] == 'usdt')
                    ...
                    return (
                      <tr
                        ...
                        style={{
                          ...
                          display: displayNone2,
                        }}
                      >
                        <td>
                          {" "}
                          <img src={addressImg} width={"20px"} /> {combineName}{" "}
                        </td>
                        <td
                          className={
                            Number(depositedLp) > 0 ? "text-bold" : "text-muted"
                          }
                        >
                          {" "}
                          {f(
                            (depositedLp / 1e18) * window.rebase_factors[0],
                            6
                          )}{" "}
                          (${f((depositedLp / 1e18) * usd_per_lp, 2)}){" "}
                        </td>
                        ...
                        >
                          {" "}
                          {f(
                            (depositedDyp / 1e18) * window.rebase_factors[0],
                            2
                          )}{" "}
                          (${f((depositedDyp / 1e18) * usd_per_token, 2)}){" "}
                        </td>
                        ...
                        >
                          {" "}
                          {f(
                            (claimableTokens * usd_per_idyp) /
                              usd_per_token /
                              1e18,
                            6
                          )}{" "}
                          (${f((claimableTokens / 1e18) * usd_per_idyp, 2)}){" "}
                        </td>
                        ...
                        >
                          {" "}
                          {f(claimableEth / 1e18, 6)} ($
                          {f((claimableEth / 1e18) * usd_per_eth, 2)}){" "}
                        </td>
                        <td
                          className={
                            Number(wethEarned) > 0 ? "text-bold" : "text-muted"
                          }
                        >
                          {" "}
                          {f(wethEarned / 1e18, 6)} ($
                          {f((wethEarned / 1e18) * usd_per_eth, 2)}){" "}
                        </td>
                        {can_access && (
                          <td
                            className={
                              Number(wethPaidOut) > 0
                                ? "text-bold"
                                : "text-muted"
                            }
                          >
                            {" "}
                            {f(wethPaidOut / 1e18, 6)} ($
                            {f((wethPaidOut / 1e18) * usd_per_eth, 2)}){" "}
                          </td>
                        )}
                        <td>
                          {" "}
                          <button
                            onClick={() => this.handleClaim(contractAddress)}
                  ...
                  <tr>
                    ...
                    <td>
                      {" "}
                      {f(
                        (this.getTotalClaimableTokens() * usd_per_idyp) /
                          usd_per_token,
                        6
                      )}{" "}
                      (${f(this.getTotalClaimableTokens() * usd_per_idyp, 2)}){" "}
                    </td>
                    <td>
                      {" "}
                      {f(this.getTotalClaimableEth(), 6)} ($
                      {f(this.getTotalClaimableEth() * usd_per_eth, 2)}){" "}
                    </td>
                    <td>
                      {" "}
                      {f(this.getCombinedWethEarnings(), 6)} ($
                      {f(this.getCombinedWethEarnings() * usd_per_eth, 2)}){" "}
                    </td>
                    {can_access && (
                      <td>
                        {" "}
                        {f(this.getTotalClaimedEth(), 6)} ($
                        {f(this.getTotalClaimedEth() * usd_per_eth, 2)}){" "}
                      </td>
                    )}
                    <td></td>
                  </tr>
